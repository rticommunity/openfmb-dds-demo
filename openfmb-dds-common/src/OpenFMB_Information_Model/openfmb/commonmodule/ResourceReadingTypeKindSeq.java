...
/**
* A sequence of ResourceReadingTypeKind instances.
*/
public final class ResourceReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ResourceReadingTypeKindSeq() {
        super(ResourceReadingTypeKind.class);
    }

    public ResourceReadingTypeKindSeq (int initialMaximum) {
        super(ResourceReadingTypeKind.class, initialMaximum);
    }

    public ResourceReadingTypeKindSeq (Collection elements) {
        super(ResourceReadingTypeKind.class, elements);
    }

    public  ResourceReadingTypeKind get(int index) { 
        return (ResourceReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ResourceReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ResourceReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ResourceReadingTypeKind.create());
