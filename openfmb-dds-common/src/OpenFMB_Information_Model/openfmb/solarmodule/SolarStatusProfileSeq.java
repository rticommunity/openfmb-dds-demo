...
/**
* A sequence of SolarStatusProfile instances.
*/
public final class SolarStatusProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarStatusProfileSeq() {
        super(SolarStatusProfile.class);
    }

    public SolarStatusProfileSeq (int initialMaximum) {
        super(SolarStatusProfile.class, initialMaximum);
    }

    public SolarStatusProfileSeq (Collection elements) {
        super(SolarStatusProfile.class, elements);
    }

    public  SolarStatusProfile get(int index) { 
        return (SolarStatusProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarStatusProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarStatusProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarStatusProfile.create());
