...
/**
* A sequence of ESSEvent instances.
*/
public final class ESSEventSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ESSEventSeq() {
        super(ESSEvent.class);
    }

    public ESSEventSeq (int initialMaximum) {
        super(ESSEvent.class, initialMaximum);
    }

    public ESSEventSeq (Collection elements) {
        super(ESSEvent.class, elements);
    }

    public  ESSEvent get(int index) { 
        return (ESSEvent) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ESSEvent.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ESSEvent objects (beyond the original size of this object)
