...
/**
* A sequence of LoadControlScheduleProfile instances.
*/
public final class LoadControlScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadControlScheduleProfileSeq() {
        super(LoadControlScheduleProfile.class);
    }

    public LoadControlScheduleProfileSeq (int initialMaximum) {
        super(LoadControlScheduleProfile.class, initialMaximum);
    }

    public LoadControlScheduleProfileSeq (Collection elements) {
        super(LoadControlScheduleProfile.class, elements);
    }

    public  LoadControlScheduleProfile get(int index) { 
        return (LoadControlScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadControlScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadControlScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadControlScheduleProfile.create());
