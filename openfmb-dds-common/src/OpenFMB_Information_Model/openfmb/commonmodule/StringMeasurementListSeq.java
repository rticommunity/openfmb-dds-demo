...
/**
* A sequence of StringMeasurementList instances.
*/
public final class StringMeasurementListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public StringMeasurementListSeq() {
        super(StringMeasurementList.class);
    }

    public StringMeasurementListSeq (int initialMaximum) {
        super(StringMeasurementList.class, initialMaximum);
    }

    public StringMeasurementListSeq (Collection elements) {
        super(StringMeasurementList.class, elements);
    }

    public  StringMeasurementList get(int index) { 
        return (StringMeasurementList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, StringMeasurementList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' StringMeasurementList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(StringMeasurementList.create());
