...
/**
* A sequence of ESSReadingProfile instances.
*/
public final class ESSReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ESSReadingProfileSeq() {
        super(ESSReadingProfile.class);
    }

    public ESSReadingProfileSeq (int initialMaximum) {
        super(ESSReadingProfile.class, initialMaximum);
    }

    public ESSReadingProfileSeq (Collection elements) {
        super(ESSReadingProfile.class, elements);
    }

    public  ESSReadingProfile get(int index) { 
        return (ESSReadingProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ESSReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ESSReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ESSReadingProfile.create());
