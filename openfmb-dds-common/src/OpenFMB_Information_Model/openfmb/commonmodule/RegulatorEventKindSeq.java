...
/**
* A sequence of RegulatorEventKind instances.
*/
public final class RegulatorEventKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorEventKindSeq() {
        super(RegulatorEventKind.class);
    }

    public RegulatorEventKindSeq (int initialMaximum) {
        super(RegulatorEventKind.class, initialMaximum);
    }

    public RegulatorEventKindSeq (Collection elements) {
        super(RegulatorEventKind.class, elements);
    }

    public  RegulatorEventKind get(int index) { 
        return (RegulatorEventKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorEventKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorEventKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorEventKind.create());
