...
/**
* A sequence of RegulatorStatusProfile instances.
*/
public final class RegulatorStatusProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorStatusProfileSeq() {
        super(RegulatorStatusProfile.class);
    }

    public RegulatorStatusProfileSeq (int initialMaximum) {
        super(RegulatorStatusProfile.class, initialMaximum);
    }

    public RegulatorStatusProfileSeq (Collection elements) {
        super(RegulatorStatusProfile.class, elements);
    }

    public  RegulatorStatusProfile get(int index) { 
        return (RegulatorStatusProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorStatusProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorStatusProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorStatusProfile.create());
