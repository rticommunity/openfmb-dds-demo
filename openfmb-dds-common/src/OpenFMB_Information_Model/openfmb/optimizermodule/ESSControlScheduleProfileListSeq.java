...
/**
* A sequence of ESSControlScheduleProfileList instances.
*/
public final class ESSControlScheduleProfileListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ESSControlScheduleProfileListSeq() {
        super(ESSControlScheduleProfileList.class);
    }

    public ESSControlScheduleProfileListSeq (int initialMaximum) {
        super(ESSControlScheduleProfileList.class, initialMaximum);
    }

    public ESSControlScheduleProfileListSeq (Collection elements) {
        super(ESSControlScheduleProfileList.class, elements);
    }

    public  ESSControlScheduleProfileList get(int index) { 
        return (ESSControlScheduleProfileList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ESSControlScheduleProfileList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ESSControlScheduleProfileList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ESSControlScheduleProfileList.create());
