...
/**
* A sequence of MotorOperatedSwitchEventProfile instances.
*/
public final class MotorOperatedSwitchEventProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchEventProfileSeq() {
        super(MotorOperatedSwitchEventProfile.class);
    }

    public MotorOperatedSwitchEventProfileSeq (int initialMaximum) {
        super(MotorOperatedSwitchEventProfile.class, initialMaximum);
    }

    public MotorOperatedSwitchEventProfileSeq (Collection elements) {
        super(MotorOperatedSwitchEventProfile.class, elements);
    }

    public  MotorOperatedSwitchEventProfile get(int index) { 
        return (MotorOperatedSwitchEventProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchEventProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchEventProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchEventProfile.create());
