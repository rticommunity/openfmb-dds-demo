...
/**
* A sequence of SolarControlScheduleList instances.
*/
public final class SolarControlScheduleListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarControlScheduleListSeq() {
        super(SolarControlScheduleList.class);
    }

    public SolarControlScheduleListSeq (int initialMaximum) {
        super(SolarControlScheduleList.class, initialMaximum);
    }

    public SolarControlScheduleListSeq (Collection elements) {
        super(SolarControlScheduleList.class, elements);
    }

    public  SolarControlScheduleList get(int index) { 
        return (SolarControlScheduleList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarControlScheduleList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarControlScheduleList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarControlScheduleList.create());
