...
/**
* A sequence of MotorOperatedSwitchStatus instances.
*/
public final class MotorOperatedSwitchStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchStatusSeq() {
        super(MotorOperatedSwitchStatus.class);
    }

    public MotorOperatedSwitchStatusSeq (int initialMaximum) {
        super(MotorOperatedSwitchStatus.class, initialMaximum);
    }

    public MotorOperatedSwitchStatusSeq (Collection elements) {
        super(MotorOperatedSwitchStatus.class, elements);
    }

    public  MotorOperatedSwitchStatus get(int index) { 
        return (MotorOperatedSwitchStatus) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchStatus.create());
