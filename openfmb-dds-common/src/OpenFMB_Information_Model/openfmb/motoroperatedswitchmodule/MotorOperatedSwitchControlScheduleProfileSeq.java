...
/**
* A sequence of MotorOperatedSwitchControlScheduleProfile instances.
*/
public final class MotorOperatedSwitchControlScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchControlScheduleProfileSeq() {
        super(MotorOperatedSwitchControlScheduleProfile.class);
    }

    public MotorOperatedSwitchControlScheduleProfileSeq (int initialMaximum) {
        super(MotorOperatedSwitchControlScheduleProfile.class, initialMaximum);
    }

    public MotorOperatedSwitchControlScheduleProfileSeq (Collection elements) {
        super(MotorOperatedSwitchControlScheduleProfile.class, elements);
    }

    public  MotorOperatedSwitchControlScheduleProfile get(int index) { 
        return (MotorOperatedSwitchControlScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchControlScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchControlScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchControlScheduleProfile.create());
