...
/**
* A sequence of ShuntControlScheduleProfile instances.
*/
public final class ShuntControlScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShuntControlScheduleProfileSeq() {
        super(ShuntControlScheduleProfile.class);
    }

    public ShuntControlScheduleProfileSeq (int initialMaximum) {
        super(ShuntControlScheduleProfile.class, initialMaximum);
    }

    public ShuntControlScheduleProfileSeq (Collection elements) {
        super(ShuntControlScheduleProfile.class, elements);
    }

    public  ShuntControlScheduleProfile get(int index) { 
        return (ShuntControlScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShuntControlScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShuntControlScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShuntControlScheduleProfile.create());
