...
/**
* A sequence of TemperatureDataList instances.
*/
public final class TemperatureDataListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TemperatureDataListSeq() {
        super(TemperatureDataList.class);
    }

    public TemperatureDataListSeq (int initialMaximum) {
        super(TemperatureDataList.class, initialMaximum);
    }

    public TemperatureDataListSeq (Collection elements) {
        super(TemperatureDataList.class, elements);
    }

    public  TemperatureDataList get(int index) { 
        return (TemperatureDataList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TemperatureDataList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TemperatureDataList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TemperatureDataList.create());
