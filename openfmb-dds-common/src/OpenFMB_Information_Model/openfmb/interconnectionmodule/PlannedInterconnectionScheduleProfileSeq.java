...
/**
* A sequence of PlannedInterconnectionScheduleProfile instances.
*/
public final class PlannedInterconnectionScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PlannedInterconnectionScheduleProfileSeq() {
        super(PlannedInterconnectionScheduleProfile.class);
    }

    public PlannedInterconnectionScheduleProfileSeq (int initialMaximum) {
        super(PlannedInterconnectionScheduleProfile.class, initialMaximum);
    }

    public PlannedInterconnectionScheduleProfileSeq (Collection elements) {
        super(PlannedInterconnectionScheduleProfile.class, elements);
    }

    public  PlannedInterconnectionScheduleProfile get(int index) { 
        return (PlannedInterconnectionScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PlannedInterconnectionScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PlannedInterconnectionScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PlannedInterconnectionScheduleProfile.create());
