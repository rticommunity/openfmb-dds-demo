...
/**
* A sequence of MotorOperatedSwitchControlScheduleList instances.
*/
public final class MotorOperatedSwitchControlScheduleListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchControlScheduleListSeq() {
        super(MotorOperatedSwitchControlScheduleList.class);
    }

    public MotorOperatedSwitchControlScheduleListSeq (int initialMaximum) {
        super(MotorOperatedSwitchControlScheduleList.class, initialMaximum);
    }

    public MotorOperatedSwitchControlScheduleListSeq (Collection elements) {
        super(MotorOperatedSwitchControlScheduleList.class, elements);
    }

    public  MotorOperatedSwitchControlScheduleList get(int index) { 
        return (MotorOperatedSwitchControlScheduleList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchControlScheduleList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchControlScheduleList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchControlScheduleList.create());
