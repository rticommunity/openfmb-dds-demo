...
/**
* A sequence of LoadReadingTypeKind instances.
*/
public final class LoadReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadReadingTypeKindSeq() {
        super(LoadReadingTypeKind.class);
    }

    public LoadReadingTypeKindSeq (int initialMaximum) {
        super(LoadReadingTypeKind.class, initialMaximum);
    }

    public LoadReadingTypeKindSeq (Collection elements) {
        super(LoadReadingTypeKind.class, elements);
    }

    public  LoadReadingTypeKind get(int index) { 
        return (LoadReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadReadingTypeKind.create());
