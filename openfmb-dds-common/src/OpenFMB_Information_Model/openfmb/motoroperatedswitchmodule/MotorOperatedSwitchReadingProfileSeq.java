...
/**
* A sequence of MotorOperatedSwitchReadingProfile instances.
*/
public final class MotorOperatedSwitchReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchReadingProfileSeq() {
        super(MotorOperatedSwitchReadingProfile.class);
    }

    public MotorOperatedSwitchReadingProfileSeq (int initialMaximum) {
        super(MotorOperatedSwitchReadingProfile.class, initialMaximum);
    }

    public MotorOperatedSwitchReadingProfileSeq (Collection elements) {
        super(MotorOperatedSwitchReadingProfile.class, elements);
    }

    public  MotorOperatedSwitchReadingProfile get(int index) { 
        return (MotorOperatedSwitchReadingProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchReadingProfile.create());
