...
/**
* A sequence of IrregularTimePointList instances.
*/
public final class IrregularTimePointListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public IrregularTimePointListSeq() {
        super(IrregularTimePointList.class);
    }

    public IrregularTimePointListSeq (int initialMaximum) {
        super(IrregularTimePointList.class, initialMaximum);
    }

    public IrregularTimePointListSeq (Collection elements) {
        super(IrregularTimePointList.class, elements);
    }

    public  IrregularTimePointList get(int index) { 
        return (IrregularTimePointList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, IrregularTimePointList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' IrregularTimePointList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(IrregularTimePointList.create());
