...
/**
* A sequence of MarketProductKind instances.
*/
public final class MarketProductKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MarketProductKindSeq() {
        super(MarketProductKind.class);
    }

    public MarketProductKindSeq (int initialMaximum) {
        super(MarketProductKind.class, initialMaximum);
    }

    public MarketProductKindSeq (Collection elements) {
        super(MarketProductKind.class, elements);
    }

    public  MarketProductKind get(int index) { 
        return (MarketProductKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MarketProductKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MarketProductKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MarketProductKind.create());
