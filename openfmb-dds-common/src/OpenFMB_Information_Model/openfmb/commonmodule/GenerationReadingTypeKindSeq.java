...
/**
* A sequence of GenerationReadingTypeKind instances.
*/
public final class GenerationReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationReadingTypeKindSeq() {
        super(GenerationReadingTypeKind.class);
    }

    public GenerationReadingTypeKindSeq (int initialMaximum) {
        super(GenerationReadingTypeKind.class, initialMaximum);
    }

    public GenerationReadingTypeKindSeq (Collection elements) {
        super(GenerationReadingTypeKind.class, elements);
    }

    public  GenerationReadingTypeKind get(int index) { 
        return (GenerationReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationReadingTypeKind.create());
