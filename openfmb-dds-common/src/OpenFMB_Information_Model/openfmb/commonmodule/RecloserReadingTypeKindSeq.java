...
/**
* A sequence of RecloserReadingTypeKind instances.
*/
public final class RecloserReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserReadingTypeKindSeq() {
        super(RecloserReadingTypeKind.class);
    }

    public RecloserReadingTypeKindSeq (int initialMaximum) {
        super(RecloserReadingTypeKind.class, initialMaximum);
    }

    public RecloserReadingTypeKindSeq (Collection elements) {
        super(RecloserReadingTypeKind.class, elements);
    }

    public  RecloserReadingTypeKind get(int index) { 
        return (RecloserReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserReadingTypeKind.create());
