...
/**
* A sequence of Breaker instances.
*/
public final class BreakerSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BreakerSeq (int initialMaximum) {
        super(Breaker.class, initialMaximum);
    }

    public BreakerSeq (Collection elements) {
        super(Breaker.class, elements);
    }

    public  Breaker get(int index) { 
        return (Breaker) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Breaker.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Breaker objects (beyond the original size of this object)
