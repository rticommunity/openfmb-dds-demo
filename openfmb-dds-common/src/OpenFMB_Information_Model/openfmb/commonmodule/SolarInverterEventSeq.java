...
/**
* A sequence of SolarInverterEvent instances.
*/
public final class SolarInverterEventSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarInverterEventSeq() {
        super(SolarInverterEvent.class);
    }

    public SolarInverterEventSeq (int initialMaximum) {
        super(SolarInverterEvent.class, initialMaximum);
    }

    public SolarInverterEventSeq (Collection elements) {
        super(SolarInverterEvent.class, elements);
    }

    public  SolarInverterEvent get(int index) { 
        return (SolarInverterEvent) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarInverterEvent.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarInverterEvent objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarInverterEvent.create());
