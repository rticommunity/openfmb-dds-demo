...
/**
* A sequence of BreakerReadingTypeKind instances.
*/
public final class BreakerReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BreakerReadingTypeKindSeq() {
        super(BreakerReadingTypeKind.class);
    }

    public BreakerReadingTypeKindSeq (int initialMaximum) {
        super(BreakerReadingTypeKind.class, initialMaximum);
    }

    public BreakerReadingTypeKindSeq (Collection elements) {
        super(BreakerReadingTypeKind.class, elements);
    }

    public  BreakerReadingTypeKind get(int index) { 
        return (BreakerReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BreakerReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BreakerReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BreakerReadingTypeKind.create());
