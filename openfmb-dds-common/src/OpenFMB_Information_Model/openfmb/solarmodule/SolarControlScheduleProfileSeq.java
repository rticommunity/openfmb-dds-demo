...
/**
* A sequence of SolarControlScheduleProfile instances.
*/
public final class SolarControlScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarControlScheduleProfileSeq() {
        super(SolarControlScheduleProfile.class);
    }

    public SolarControlScheduleProfileSeq (int initialMaximum) {
        super(SolarControlScheduleProfile.class, initialMaximum);
    }

    public SolarControlScheduleProfileSeq (Collection elements) {
        super(SolarControlScheduleProfile.class, elements);
    }

    public  SolarControlScheduleProfile get(int index) { 
        return (SolarControlScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarControlScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarControlScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarControlScheduleProfile.create());
