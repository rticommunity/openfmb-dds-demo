...
/**
* A sequence of AnalogMeasurementList instances.
*/
public final class AnalogMeasurementListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public AnalogMeasurementListSeq() {
        super(AnalogMeasurementList.class);
    }

    public AnalogMeasurementListSeq (int initialMaximum) {
        super(AnalogMeasurementList.class, initialMaximum);
    }

    public AnalogMeasurementListSeq (Collection elements) {
        super(AnalogMeasurementList.class, elements);
    }

    public  AnalogMeasurementList get(int index) { 
        return (AnalogMeasurementList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, AnalogMeasurementList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' AnalogMeasurementList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(AnalogMeasurementList.create());
