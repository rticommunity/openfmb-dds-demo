...
/**
* A sequence of LoadControlScheduleList instances.
*/
public final class LoadControlScheduleListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadControlScheduleListSeq() {
        super(LoadControlScheduleList.class);
    }

    public LoadControlScheduleListSeq (int initialMaximum) {
        super(LoadControlScheduleList.class, initialMaximum);
    }

    public LoadControlScheduleListSeq (Collection elements) {
        super(LoadControlScheduleList.class, elements);
    }

    public  LoadControlScheduleList get(int index) { 
        return (LoadControlScheduleList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadControlScheduleList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadControlScheduleList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadControlScheduleList.create());
