...
/**
* A sequence of BooleanMeasurement instances.
*/
public final class BooleanMeasurementSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BooleanMeasurementSeq() {
        super(BooleanMeasurement.class);
    }

    public BooleanMeasurementSeq (int initialMaximum) {
        super(BooleanMeasurement.class, initialMaximum);
    }

    public BooleanMeasurementSeq (Collection elements) {
        super(BooleanMeasurement.class, elements);
    }

    public  BooleanMeasurement get(int index) { 
        return (BooleanMeasurement) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BooleanMeasurement.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BooleanMeasurement objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BooleanMeasurement.create());
