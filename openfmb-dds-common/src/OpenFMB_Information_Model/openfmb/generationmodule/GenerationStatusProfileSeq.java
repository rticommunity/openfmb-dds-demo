...
/**
* A sequence of GenerationStatusProfile instances.
*/
public final class GenerationStatusProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationStatusProfileSeq() {
        super(GenerationStatusProfile.class);
    }

    public GenerationStatusProfileSeq (int initialMaximum) {
        super(GenerationStatusProfile.class, initialMaximum);
    }

    public GenerationStatusProfileSeq (Collection elements) {
        super(GenerationStatusProfile.class, elements);
    }

    public  GenerationStatusProfile get(int index) { 
        return (GenerationStatusProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationStatusProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationStatusProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationStatusProfile.create());
