...
/**
* A sequence of MotorOperatedSwitchStatusProfile instances.
*/
public final class MotorOperatedSwitchStatusProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchStatusProfileSeq() {
        super(MotorOperatedSwitchStatusProfile.class);
    }

    public MotorOperatedSwitchStatusProfileSeq (int initialMaximum) {
        super(MotorOperatedSwitchStatusProfile.class, initialMaximum);
    }

    public MotorOperatedSwitchStatusProfileSeq (Collection elements) {
        super(MotorOperatedSwitchStatusProfile.class, elements);
    }

    public  MotorOperatedSwitchStatusProfile get(int index) { 
        return (MotorOperatedSwitchStatusProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchStatusProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchStatusProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchStatusProfile.create());
