...
/**
* A sequence of ESSReadingTypeKind instances.
*/
public final class ESSReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ESSReadingTypeKindSeq() {
        super(ESSReadingTypeKind.class);
    }

    public ESSReadingTypeKindSeq (int initialMaximum) {
        super(ESSReadingTypeKind.class, initialMaximum);
    }

    public ESSReadingTypeKindSeq (Collection elements) {
        super(ESSReadingTypeKind.class, elements);
    }

    public  ESSReadingTypeKind get(int index) { 
        return (ESSReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ESSReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ESSReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ESSReadingTypeKind.create());
