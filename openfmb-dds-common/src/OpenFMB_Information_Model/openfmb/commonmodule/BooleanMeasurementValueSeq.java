...
/**
* A sequence of BooleanMeasurementValue instances.
*/
public final class BooleanMeasurementValueSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BooleanMeasurementValueSeq() {
        super(BooleanMeasurementValue.class);
    }

    public BooleanMeasurementValueSeq (int initialMaximum) {
        super(BooleanMeasurementValue.class, initialMaximum);
    }

    public BooleanMeasurementValueSeq (Collection elements) {
        super(BooleanMeasurementValue.class, elements);
    }

    public  BooleanMeasurementValue get(int index) { 
        return (BooleanMeasurementValue) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BooleanMeasurementValue.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BooleanMeasurementValue objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BooleanMeasurementValue.create());
