...
/**
* A sequence of GenerationReading instances.
*/
public final class GenerationReadingSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationReadingSeq() {
        super(GenerationReading.class);
    }

    public GenerationReadingSeq (int initialMaximum) {
        super(GenerationReading.class, initialMaximum);
    }

    public GenerationReadingSeq (Collection elements) {
        super(GenerationReading.class, elements);
    }

    public  GenerationReading get(int index) { 
        return (GenerationReading) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationReading.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationReading objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationReading.create());
