...
/**
* A sequence of MotorOperatedSwitchEvent instances.
*/
public final class MotorOperatedSwitchEventSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchEventSeq() {
        super(MotorOperatedSwitchEvent.class);
    }

    public MotorOperatedSwitchEventSeq (int initialMaximum) {
        super(MotorOperatedSwitchEvent.class, initialMaximum);
    }

    public MotorOperatedSwitchEventSeq (Collection elements) {
        super(MotorOperatedSwitchEvent.class, elements);
    }

    public  MotorOperatedSwitchEvent get(int index) { 
        return (MotorOperatedSwitchEvent) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchEvent.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchEvent objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchEvent.create());
