...
/**
* A sequence of MotorOperatedSwitch instances.
*/
public final class MotorOperatedSwitchSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchSeq() {
        super(MotorOperatedSwitch.class);
    }

    public MotorOperatedSwitchSeq (int initialMaximum) {
        super(MotorOperatedSwitch.class, initialMaximum);
    }

    public MotorOperatedSwitchSeq (Collection elements) {
        super(MotorOperatedSwitch.class, elements);
    }

    public  MotorOperatedSwitch get(int index) { 
        return (MotorOperatedSwitch) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitch.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitch objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitch.create());
