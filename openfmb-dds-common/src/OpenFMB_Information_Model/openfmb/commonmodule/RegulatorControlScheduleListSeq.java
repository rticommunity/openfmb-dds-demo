...
/**
* A sequence of RegulatorControlScheduleList instances.
*/
public final class RegulatorControlScheduleListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorControlScheduleListSeq() {
        super(RegulatorControlScheduleList.class);
    }

    public RegulatorControlScheduleListSeq (int initialMaximum) {
        super(RegulatorControlScheduleList.class, initialMaximum);
    }

    public RegulatorControlScheduleListSeq (Collection elements) {
        super(RegulatorControlScheduleList.class, elements);
    }

    public  RegulatorControlScheduleList get(int index) { 
        return (RegulatorControlScheduleList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorControlScheduleList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorControlScheduleList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorControlScheduleList.create());
