...
/**
* A sequence of RegulatorEventProfile instances.
*/
public final class RegulatorEventProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorEventProfileSeq() {
        super(RegulatorEventProfile.class);
    }

    public RegulatorEventProfileSeq (int initialMaximum) {
        super(RegulatorEventProfile.class, initialMaximum);
    }

    public RegulatorEventProfileSeq (Collection elements) {
        super(RegulatorEventProfile.class, elements);
    }

    public  RegulatorEventProfile get(int index) { 
        return (RegulatorEventProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorEventProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorEventProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorEventProfile.create());
