...
/**
* A sequence of GenerationEventKind instances.
*/
public final class GenerationEventKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationEventKindSeq() {
        super(GenerationEventKind.class);
    }

    public GenerationEventKindSeq (int initialMaximum) {
        super(GenerationEventKind.class, initialMaximum);
    }

    public GenerationEventKindSeq (Collection elements) {
        super(GenerationEventKind.class, elements);
    }

    public  GenerationEventKind get(int index) { 
        return (GenerationEventKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationEventKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationEventKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationEventKind.create());
