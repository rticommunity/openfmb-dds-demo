...
/**
* A sequence of PlannedInterconnectionScheduleProfileList instances.
*/
public final class PlannedInterconnectionScheduleProfileListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PlannedInterconnectionScheduleProfileListSeq() {
        super(PlannedInterconnectionScheduleProfileList.class);
    }

    public PlannedInterconnectionScheduleProfileListSeq (int initialMaximum) {
        super(PlannedInterconnectionScheduleProfileList.class, initialMaximum);
    }

    public PlannedInterconnectionScheduleProfileListSeq (Collection elements) {
        super(PlannedInterconnectionScheduleProfileList.class, elements);
    }

    public  PlannedInterconnectionScheduleProfileList get(int index) { 
        return (PlannedInterconnectionScheduleProfileList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PlannedInterconnectionScheduleProfileList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PlannedInterconnectionScheduleProfileList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PlannedInterconnectionScheduleProfileList.create());
