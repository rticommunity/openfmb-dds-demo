...
/**
* A sequence of ESSControlScheduleProfile instances.
*/
public final class ESSControlScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ESSControlScheduleProfileSeq() {
        super(ESSControlScheduleProfile.class);
    }

    public ESSControlScheduleProfileSeq (int initialMaximum) {
        super(ESSControlScheduleProfile.class, initialMaximum);
    }

    public ESSControlScheduleProfileSeq (Collection elements) {
        super(ESSControlScheduleProfile.class, elements);
    }

    public  ESSControlScheduleProfile get(int index) { 
        return (ESSControlScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ESSControlScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ESSControlScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ESSControlScheduleProfile.create());
