...
/**
* A sequence of RequestedInterconnectionScheduleProfile instances.
*/
public final class RequestedInterconnectionScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RequestedInterconnectionScheduleProfileSeq() {
        super(RequestedInterconnectionScheduleProfile.class);
    }

    public RequestedInterconnectionScheduleProfileSeq (int initialMaximum) {
        super(RequestedInterconnectionScheduleProfile.class, initialMaximum);
    }

    public RequestedInterconnectionScheduleProfileSeq (Collection elements) {
        super(RequestedInterconnectionScheduleProfile.class, elements);
    }

    public  RequestedInterconnectionScheduleProfile get(int index) { 
        return (RequestedInterconnectionScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RequestedInterconnectionScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RequestedInterconnectionScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RequestedInterconnectionScheduleProfile.create());
