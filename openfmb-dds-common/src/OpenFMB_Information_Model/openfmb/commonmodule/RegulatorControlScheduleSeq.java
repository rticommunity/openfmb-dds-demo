...
/**
* A sequence of RegulatorControlSchedule instances.
*/
public final class RegulatorControlScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorControlScheduleSeq() {
        super(RegulatorControlSchedule.class);
    }

    public RegulatorControlScheduleSeq (int initialMaximum) {
        super(RegulatorControlSchedule.class, initialMaximum);
    }

    public RegulatorControlScheduleSeq (Collection elements) {
        super(RegulatorControlSchedule.class, elements);
    }

    public  RegulatorControlSchedule get(int index) { 
        return (RegulatorControlSchedule) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorControlSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorControlSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorControlSchedule.create());
