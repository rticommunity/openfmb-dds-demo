...
/**
* A sequence of RequestedOptimizerScheduleProfile instances.
*/
public final class RequestedOptimizerScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RequestedOptimizerScheduleProfileSeq() {
        super(RequestedOptimizerScheduleProfile.class);
    }

    public RequestedOptimizerScheduleProfileSeq (int initialMaximum) {
        super(RequestedOptimizerScheduleProfile.class, initialMaximum);
    }

    public RequestedOptimizerScheduleProfileSeq (Collection elements) {
        super(RequestedOptimizerScheduleProfile.class, elements);
    }

    public  RequestedOptimizerScheduleProfile get(int index) { 
        return (RequestedOptimizerScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RequestedOptimizerScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RequestedOptimizerScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RequestedOptimizerScheduleProfile.create());
