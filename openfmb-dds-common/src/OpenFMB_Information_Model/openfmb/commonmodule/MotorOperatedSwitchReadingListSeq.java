...
/**
* A sequence of MotorOperatedSwitchReadingList instances.
*/
public final class MotorOperatedSwitchReadingListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchReadingListSeq() {
        super(MotorOperatedSwitchReadingList.class);
    }

    public MotorOperatedSwitchReadingListSeq (int initialMaximum) {
        super(MotorOperatedSwitchReadingList.class, initialMaximum);
    }

    public MotorOperatedSwitchReadingListSeq (Collection elements) {
        super(MotorOperatedSwitchReadingList.class, elements);
    }

    public  MotorOperatedSwitchReadingList get(int index) { 
        return (MotorOperatedSwitchReadingList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchReadingList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchReadingList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchReadingList.create());
