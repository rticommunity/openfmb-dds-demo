...
/**
* A sequence of ESSControlScheduleList instances.
*/
public final class ESSControlScheduleListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ESSControlScheduleListSeq() {
        super(ESSControlScheduleList.class);
    }

    public ESSControlScheduleListSeq (int initialMaximum) {
        super(ESSControlScheduleList.class, initialMaximum);
    }

    public ESSControlScheduleListSeq (Collection elements) {
        super(ESSControlScheduleList.class, elements);
    }

    public  ESSControlScheduleList get(int index) { 
        return (ESSControlScheduleList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ESSControlScheduleList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ESSControlScheduleList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ESSControlScheduleList.create());
