...
/**
* A sequence of DiscreteMeasurementList instances.
*/
public final class DiscreteMeasurementListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DiscreteMeasurementListSeq() {
        super(DiscreteMeasurementList.class);
    }

    public DiscreteMeasurementListSeq (int initialMaximum) {
        super(DiscreteMeasurementList.class, initialMaximum);
    }

    public DiscreteMeasurementListSeq (Collection elements) {
        super(DiscreteMeasurementList.class, elements);
    }

    public  DiscreteMeasurementList get(int index) { 
        return (DiscreteMeasurementList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DiscreteMeasurementList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DiscreteMeasurementList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DiscreteMeasurementList.create());
