...
/**
* A sequence of MotorOperatedSwitchReadingTypeKind instances.
*/
public final class MotorOperatedSwitchReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchReadingTypeKindSeq() {
        super(MotorOperatedSwitchReadingTypeKind.class);
    }

    public MotorOperatedSwitchReadingTypeKindSeq (int initialMaximum) {
        super(MotorOperatedSwitchReadingTypeKind.class, initialMaximum);
    }

    public MotorOperatedSwitchReadingTypeKindSeq (Collection elements) {
        super(MotorOperatedSwitchReadingTypeKind.class, elements);
    }

    public  MotorOperatedSwitchReadingTypeKind get(int index) { 
        return (MotorOperatedSwitchReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchReadingTypeKind.create());
