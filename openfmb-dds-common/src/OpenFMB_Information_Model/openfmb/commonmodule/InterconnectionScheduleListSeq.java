...
/**
* A sequence of InterconnectionScheduleList instances.
*/
public final class InterconnectionScheduleListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InterconnectionScheduleListSeq() {
        super(InterconnectionScheduleList.class);
    }

    public InterconnectionScheduleListSeq (int initialMaximum) {
        super(InterconnectionScheduleList.class, initialMaximum);
    }

    public InterconnectionScheduleListSeq (Collection elements) {
        super(InterconnectionScheduleList.class, elements);
    }

    public  InterconnectionScheduleList get(int index) { 
        return (InterconnectionScheduleList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InterconnectionScheduleList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InterconnectionScheduleList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InterconnectionScheduleList.create());
