...
/**
* A sequence of InterchangeScheduleList instances.
*/
public final class InterchangeScheduleListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InterchangeScheduleListSeq() {
        super(InterchangeScheduleList.class);
    }

    public InterchangeScheduleListSeq (int initialMaximum) {
        super(InterchangeScheduleList.class, initialMaximum);
    }

    public InterchangeScheduleListSeq (Collection elements) {
        super(InterchangeScheduleList.class, elements);
    }

    public  InterchangeScheduleList get(int index) { 
        return (InterchangeScheduleList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InterchangeScheduleList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InterchangeScheduleList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InterchangeScheduleList.create());
