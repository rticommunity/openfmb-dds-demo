...
/**
* A sequence of ShuntControlScheduleList instances.
*/
public final class ShuntControlScheduleListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShuntControlScheduleListSeq() {
        super(ShuntControlScheduleList.class);
    }

    public ShuntControlScheduleListSeq (int initialMaximum) {
        super(ShuntControlScheduleList.class, initialMaximum);
    }

    public ShuntControlScheduleListSeq (Collection elements) {
        super(ShuntControlScheduleList.class, elements);
    }

    public  ShuntControlScheduleList get(int index) { 
        return (ShuntControlScheduleList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShuntControlScheduleList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShuntControlScheduleList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShuntControlScheduleList.create());
