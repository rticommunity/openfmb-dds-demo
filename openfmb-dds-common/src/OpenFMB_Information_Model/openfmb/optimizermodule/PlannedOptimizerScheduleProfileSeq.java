...
/**
* A sequence of PlannedOptimizerScheduleProfile instances.
*/
public final class PlannedOptimizerScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PlannedOptimizerScheduleProfileSeq() {
        super(PlannedOptimizerScheduleProfile.class);
    }

    public PlannedOptimizerScheduleProfileSeq (int initialMaximum) {
        super(PlannedOptimizerScheduleProfile.class, initialMaximum);
    }

    public PlannedOptimizerScheduleProfileSeq (Collection elements) {
        super(PlannedOptimizerScheduleProfile.class, elements);
    }

    public  PlannedOptimizerScheduleProfile get(int index) { 
        return (PlannedOptimizerScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PlannedOptimizerScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PlannedOptimizerScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PlannedOptimizerScheduleProfile.create());
