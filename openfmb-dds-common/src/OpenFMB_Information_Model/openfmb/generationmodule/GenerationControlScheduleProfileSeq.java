...
/**
* A sequence of GenerationControlScheduleProfile instances.
*/
public final class GenerationControlScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationControlScheduleProfileSeq() {
        super(GenerationControlScheduleProfile.class);
    }

    public GenerationControlScheduleProfileSeq (int initialMaximum) {
        super(GenerationControlScheduleProfile.class, initialMaximum);
    }

    public GenerationControlScheduleProfileSeq (Collection elements) {
        super(GenerationControlScheduleProfile.class, elements);
    }

    public  GenerationControlScheduleProfile get(int index) { 
        return (GenerationControlScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationControlScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationControlScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationControlScheduleProfile.create());
