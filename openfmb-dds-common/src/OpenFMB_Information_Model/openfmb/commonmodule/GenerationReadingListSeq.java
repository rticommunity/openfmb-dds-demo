...
/**
* A sequence of GenerationReadingList instances.
*/
public final class GenerationReadingListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationReadingListSeq() {
        super(GenerationReadingList.class);
    }

    public GenerationReadingListSeq (int initialMaximum) {
        super(GenerationReadingList.class, initialMaximum);
    }

    public GenerationReadingListSeq (Collection elements) {
        super(GenerationReadingList.class, elements);
    }

    public  GenerationReadingList get(int index) { 
        return (GenerationReadingList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationReadingList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationReadingList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationReadingList.create());
