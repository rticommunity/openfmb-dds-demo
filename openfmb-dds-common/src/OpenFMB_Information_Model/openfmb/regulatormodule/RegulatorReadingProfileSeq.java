...
/**
* A sequence of RegulatorReadingProfile instances.
*/
public final class RegulatorReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorReadingProfileSeq() {
        super(RegulatorReadingProfile.class);
    }

    public RegulatorReadingProfileSeq (int initialMaximum) {
        super(RegulatorReadingProfile.class, initialMaximum);
    }

    public RegulatorReadingProfileSeq (Collection elements) {
        super(RegulatorReadingProfile.class, elements);
    }

    public  RegulatorReadingProfile get(int index) { 
        return (RegulatorReadingProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorReadingProfile.create());
