...
/**
* A sequence of RequestedInterconnectionScheduleProfileList instances.
*/
public final class RequestedInterconnectionScheduleProfileListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RequestedInterconnectionScheduleProfileListSeq() {
        super(RequestedInterconnectionScheduleProfileList.class);
    }

    public RequestedInterconnectionScheduleProfileListSeq (int initialMaximum) {
        super(RequestedInterconnectionScheduleProfileList.class, initialMaximum);
    }

    public RequestedInterconnectionScheduleProfileListSeq (Collection elements) {
        super(RequestedInterconnectionScheduleProfileList.class, elements);
    }

    public  RequestedInterconnectionScheduleProfileList get(int index) { 
        return (RequestedInterconnectionScheduleProfileList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RequestedInterconnectionScheduleProfileList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RequestedInterconnectionScheduleProfileList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RequestedInterconnectionScheduleProfileList.create());
