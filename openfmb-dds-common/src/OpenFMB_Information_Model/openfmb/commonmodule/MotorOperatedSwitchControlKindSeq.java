...
/**
* A sequence of MotorOperatedSwitchControlKind instances.
*/
public final class MotorOperatedSwitchControlKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchControlKindSeq() {
        super(MotorOperatedSwitchControlKind.class);
    }

    public MotorOperatedSwitchControlKindSeq (int initialMaximum) {
        super(MotorOperatedSwitchControlKind.class, initialMaximum);
    }

    public MotorOperatedSwitchControlKindSeq (Collection elements) {
        super(MotorOperatedSwitchControlKind.class, elements);
    }

    public  MotorOperatedSwitchControlKind get(int index) { 
        return (MotorOperatedSwitchControlKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchControlKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchControlKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchControlKind.create());
