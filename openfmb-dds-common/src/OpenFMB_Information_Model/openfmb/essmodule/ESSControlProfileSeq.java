...
/**
* A sequence of ESSControlProfile instances.
*/
public final class ESSControlProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ESSControlProfileSeq() {
        super(ESSControlProfile.class);
    }

    public ESSControlProfileSeq (int initialMaximum) {
        super(ESSControlProfile.class, initialMaximum);
    }

    public ESSControlProfileSeq (Collection elements) {
        super(ESSControlProfile.class, elements);
    }

    public  ESSControlProfile get(int index) { 
        return (ESSControlProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ESSControlProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ESSControlProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ESSControlProfile.create());
