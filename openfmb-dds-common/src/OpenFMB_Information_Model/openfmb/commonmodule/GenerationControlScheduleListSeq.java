...
/**
* A sequence of GenerationControlScheduleList instances.
*/
public final class GenerationControlScheduleListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationControlScheduleListSeq() {
        super(GenerationControlScheduleList.class);
    }

    public GenerationControlScheduleListSeq (int initialMaximum) {
        super(GenerationControlScheduleList.class, initialMaximum);
    }

    public GenerationControlScheduleListSeq (Collection elements) {
        super(GenerationControlScheduleList.class, elements);
    }

    public  GenerationControlScheduleList get(int index) { 
        return (GenerationControlScheduleList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationControlScheduleList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationControlScheduleList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationControlScheduleList.create());
