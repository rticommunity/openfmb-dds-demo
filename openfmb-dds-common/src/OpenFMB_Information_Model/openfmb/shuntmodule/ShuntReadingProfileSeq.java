...
/**
* A sequence of ShuntReadingProfile instances.
*/
public final class ShuntReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShuntReadingProfileSeq() {
        super(ShuntReadingProfile.class);
    }

    public ShuntReadingProfileSeq (int initialMaximum) {
        super(ShuntReadingProfile.class, initialMaximum);
    }

    public ShuntReadingProfileSeq (Collection elements) {
        super(ShuntReadingProfile.class, elements);
    }

    public  ShuntReadingProfile get(int index) { 
        return (ShuntReadingProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShuntReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShuntReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShuntReadingProfile.create());
