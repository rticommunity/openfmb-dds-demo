...
/**
* A sequence of BreakerReadingList instances.
*/
public final class BreakerReadingListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BreakerReadingListSeq() {
        super(BreakerReadingList.class);
    }

    public BreakerReadingListSeq (int initialMaximum) {
        super(BreakerReadingList.class, initialMaximum);
    }

    public BreakerReadingListSeq (Collection elements) {
        super(BreakerReadingList.class, elements);
    }

    public  BreakerReadingList get(int index) { 
        return (BreakerReadingList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BreakerReadingList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BreakerReadingList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BreakerReadingList.create());
