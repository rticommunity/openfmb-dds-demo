...
/**
* A sequence of ShuntReadingTypeKind instances.
*/
public final class ShuntReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShuntReadingTypeKindSeq() {
        super(ShuntReadingTypeKind.class);
    }

    public ShuntReadingTypeKindSeq (int initialMaximum) {
        super(ShuntReadingTypeKind.class, initialMaximum);
    }

    public ShuntReadingTypeKindSeq (Collection elements) {
        super(ShuntReadingTypeKind.class, elements);
    }

    public  ShuntReadingTypeKind get(int index) { 
        return (ShuntReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShuntReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShuntReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShuntReadingTypeKind.create());
