...
/**
* A sequence of InterconnectionSchedule instances.
*/
public final class InterconnectionScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InterconnectionScheduleSeq() {
        super(InterconnectionSchedule.class);
    }

    public InterconnectionScheduleSeq (int initialMaximum) {
        super(InterconnectionSchedule.class, initialMaximum);
    }

    public InterconnectionScheduleSeq (Collection elements) {
        super(InterconnectionSchedule.class, elements);
    }

    public  InterconnectionSchedule get(int index) { 
        return (InterconnectionSchedule) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InterconnectionSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InterconnectionSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InterconnectionSchedule.create());
