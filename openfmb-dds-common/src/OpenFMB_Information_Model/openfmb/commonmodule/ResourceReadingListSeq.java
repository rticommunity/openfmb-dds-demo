...
/**
* A sequence of ResourceReadingList instances.
*/
public final class ResourceReadingListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ResourceReadingListSeq() {
        super(ResourceReadingList.class);
    }

    public ResourceReadingListSeq (int initialMaximum) {
        super(ResourceReadingList.class, initialMaximum);
    }

    public ResourceReadingListSeq (Collection elements) {
        super(ResourceReadingList.class, elements);
    }

    public  ResourceReadingList get(int index) { 
        return (ResourceReadingList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ResourceReadingList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ResourceReadingList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ResourceReadingList.create());
