...
/**
* A sequence of SolarReadingTypeKind instances.
*/
public final class SolarReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarReadingTypeKindSeq() {
        super(SolarReadingTypeKind.class);
    }

    public SolarReadingTypeKindSeq (int initialMaximum) {
        super(SolarReadingTypeKind.class, initialMaximum);
    }

    public SolarReadingTypeKindSeq (Collection elements) {
        super(SolarReadingTypeKind.class, elements);
    }

    public  SolarReadingTypeKind get(int index) { 
        return (SolarReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarReadingTypeKind.create());
