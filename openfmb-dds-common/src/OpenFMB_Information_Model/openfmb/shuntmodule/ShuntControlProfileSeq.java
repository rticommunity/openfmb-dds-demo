...
/**
* A sequence of ShuntControlProfile instances.
*/
public final class ShuntControlProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShuntControlProfileSeq() {
        super(ShuntControlProfile.class);
    }

    public ShuntControlProfileSeq (int initialMaximum) {
        super(ShuntControlProfile.class, initialMaximum);
    }

    public ShuntControlProfileSeq (Collection elements) {
        super(ShuntControlProfile.class, elements);
    }

    public  ShuntControlProfile get(int index) { 
        return (ShuntControlProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShuntControlProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShuntControlProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShuntControlProfile.create());
