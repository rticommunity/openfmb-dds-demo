...
/**
* A sequence of GenerationControlKind instances.
*/
public final class GenerationControlKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationControlKindSeq() {
        super(GenerationControlKind.class);
    }

    public GenerationControlKindSeq (int initialMaximum) {
        super(GenerationControlKind.class, initialMaximum);
    }

    public GenerationControlKindSeq (Collection elements) {
        super(GenerationControlKind.class, elements);
    }

    public  GenerationControlKind get(int index) { 
        return (GenerationControlKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationControlKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationControlKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationControlKind.create());
