...
/**
* A sequence of ESSControlSchedule instances.
*/
public final class ESSControlScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ESSControlScheduleSeq() {
        super(ESSControlSchedule.class);
    }

    public ESSControlScheduleSeq (int initialMaximum) {
        super(ESSControlSchedule.class, initialMaximum);
    }

    public ESSControlScheduleSeq (Collection elements) {
        super(ESSControlSchedule.class, elements);
    }

    public  ESSControlSchedule get(int index) { 
        return (ESSControlSchedule) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ESSControlSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ESSControlSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ESSControlSchedule.create());
