...
/**
* A sequence of GenerationControlSchedule instances.
*/
public final class GenerationControlScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GenerationControlScheduleSeq() {
        super(GenerationControlSchedule.class);
    }

    public GenerationControlScheduleSeq (int initialMaximum) {
        super(GenerationControlSchedule.class, initialMaximum);
    }

    public GenerationControlScheduleSeq (Collection elements) {
        super(GenerationControlSchedule.class, elements);
    }

    public  GenerationControlSchedule get(int index) { 
        return (GenerationControlSchedule) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GenerationControlSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GenerationControlSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GenerationControlSchedule.create());
