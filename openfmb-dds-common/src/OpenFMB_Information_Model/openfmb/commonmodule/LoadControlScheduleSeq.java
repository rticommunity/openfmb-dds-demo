...
/**
* A sequence of LoadControlSchedule instances.
*/
public final class LoadControlScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadControlScheduleSeq() {
        super(LoadControlSchedule.class);
    }

    public LoadControlScheduleSeq (int initialMaximum) {
        super(LoadControlSchedule.class, initialMaximum);
    }

    public LoadControlScheduleSeq (Collection elements) {
        super(LoadControlSchedule.class, elements);
    }

    public  LoadControlSchedule get(int index) { 
        return (LoadControlSchedule) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadControlSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadControlSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadControlSchedule.create());
