...
/**
* A sequence of EnergyProductKind instances.
*/
public final class EnergyProductKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public EnergyProductKindSeq() {
        super(EnergyProductKind.class);
    }

    public EnergyProductKindSeq (int initialMaximum) {
        super(EnergyProductKind.class, initialMaximum);
    }

    public EnergyProductKindSeq (Collection elements) {
        super(EnergyProductKind.class, elements);
    }

    public  EnergyProductKind get(int index) { 
        return (EnergyProductKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, EnergyProductKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' EnergyProductKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(EnergyProductKind.create());
