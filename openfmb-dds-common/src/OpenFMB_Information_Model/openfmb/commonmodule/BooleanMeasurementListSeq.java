...
/**
* A sequence of BooleanMeasurementList instances.
*/
public final class BooleanMeasurementListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BooleanMeasurementListSeq() {
        super(BooleanMeasurementList.class);
    }

    public BooleanMeasurementListSeq (int initialMaximum) {
        super(BooleanMeasurementList.class, initialMaximum);
    }

    public BooleanMeasurementListSeq (Collection elements) {
        super(BooleanMeasurementList.class, elements);
    }

    public  BooleanMeasurementList get(int index) { 
        return (BooleanMeasurementList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BooleanMeasurementList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BooleanMeasurementList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BooleanMeasurementList.create());
