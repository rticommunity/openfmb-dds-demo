...
/**
* A sequence of RecloserControlKind instances.
*/
public final class RecloserControlKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserControlKindSeq() {
        super(RecloserControlKind.class);
    }

    public RecloserControlKindSeq (int initialMaximum) {
        super(RecloserControlKind.class, initialMaximum);
    }

    public RecloserControlKindSeq (Collection elements) {
        super(RecloserControlKind.class, elements);
    }

    public  RecloserControlKind get(int index) { 
        return (RecloserControlKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserControlKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserControlKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserControlKind.create());
