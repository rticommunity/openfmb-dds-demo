...
/**
* A sequence of LoadControlScheduleProfileList instances.
*/
public final class LoadControlScheduleProfileListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LoadControlScheduleProfileListSeq() {
        super(LoadControlScheduleProfileList.class);
    }

    public LoadControlScheduleProfileListSeq (int initialMaximum) {
        super(LoadControlScheduleProfileList.class, initialMaximum);
    }

    public LoadControlScheduleProfileListSeq (Collection elements) {
        super(LoadControlScheduleProfileList.class, elements);
    }

    public  LoadControlScheduleProfileList get(int index) { 
        return (LoadControlScheduleProfileList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LoadControlScheduleProfileList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LoadControlScheduleProfileList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LoadControlScheduleProfileList.create());
