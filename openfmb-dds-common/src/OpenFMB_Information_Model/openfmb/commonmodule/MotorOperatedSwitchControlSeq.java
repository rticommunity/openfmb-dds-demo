...
/**
* A sequence of MotorOperatedSwitchControl instances.
*/
public final class MotorOperatedSwitchControlSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchControlSeq() {
        super(MotorOperatedSwitchControl.class);
    }

    public MotorOperatedSwitchControlSeq (int initialMaximum) {
        super(MotorOperatedSwitchControl.class, initialMaximum);
    }

    public MotorOperatedSwitchControlSeq (Collection elements) {
        super(MotorOperatedSwitchControl.class, elements);
    }

    public  MotorOperatedSwitchControl get(int index) { 
        return (MotorOperatedSwitchControl) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchControl.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchControl objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchControl.create());
