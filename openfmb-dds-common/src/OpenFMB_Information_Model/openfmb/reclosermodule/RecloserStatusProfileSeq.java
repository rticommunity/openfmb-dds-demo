...
/**
* A sequence of RecloserStatusProfile instances.
*/
public final class RecloserStatusProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserStatusProfileSeq() {
        super(RecloserStatusProfile.class);
    }

    public RecloserStatusProfileSeq (int initialMaximum) {
        super(RecloserStatusProfile.class, initialMaximum);
    }

    public RecloserStatusProfileSeq (Collection elements) {
        super(RecloserStatusProfile.class, elements);
    }

    public  RecloserStatusProfile get(int index) { 
        return (RecloserStatusProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserStatusProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserStatusProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserStatusProfile.create());
