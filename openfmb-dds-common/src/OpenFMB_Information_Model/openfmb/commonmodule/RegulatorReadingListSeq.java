...
/**
* A sequence of RegulatorReadingList instances.
*/
public final class RegulatorReadingListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorReadingListSeq() {
        super(RegulatorReadingList.class);
    }

    public RegulatorReadingListSeq (int initialMaximum) {
        super(RegulatorReadingList.class, initialMaximum);
    }

    public RegulatorReadingListSeq (Collection elements) {
        super(RegulatorReadingList.class, elements);
    }

    public  RegulatorReadingList get(int index) { 
        return (RegulatorReadingList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorReadingList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorReadingList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorReadingList.create());
