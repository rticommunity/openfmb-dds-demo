...
/**
* A sequence of BreakerStatusProfile instances.
*/
public final class BreakerStatusProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BreakerStatusProfileSeq() {
        super(BreakerStatusProfile.class);
    }

    public BreakerStatusProfileSeq (int initialMaximum) {
        super(BreakerStatusProfile.class, initialMaximum);
    }

    public BreakerStatusProfileSeq (Collection elements) {
        super(BreakerStatusProfile.class, elements);
    }

    public  BreakerStatusProfile get(int index) { 
        return (BreakerStatusProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BreakerStatusProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BreakerStatusProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BreakerStatusProfile.create());
