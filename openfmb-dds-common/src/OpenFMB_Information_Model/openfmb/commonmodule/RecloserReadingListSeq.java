...
/**
* A sequence of RecloserReadingList instances.
*/
public final class RecloserReadingListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RecloserReadingListSeq() {
        super(RecloserReadingList.class);
    }

    public RecloserReadingListSeq (int initialMaximum) {
        super(RecloserReadingList.class, initialMaximum);
    }

    public RecloserReadingListSeq (Collection elements) {
        super(RecloserReadingList.class, elements);
    }

    public  RecloserReadingList get(int index) { 
        return (RecloserReadingList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RecloserReadingList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RecloserReadingList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RecloserReadingList.create());
