...
/**
* A sequence of RegulatorControlScheduleProfile instances.
*/
public final class RegulatorControlScheduleProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorControlScheduleProfileSeq() {
        super(RegulatorControlScheduleProfile.class);
    }

    public RegulatorControlScheduleProfileSeq (int initialMaximum) {
        super(RegulatorControlScheduleProfile.class, initialMaximum);
    }

    public RegulatorControlScheduleProfileSeq (Collection elements) {
        super(RegulatorControlScheduleProfile.class, elements);
    }

    public  RegulatorControlScheduleProfile get(int index) { 
        return (RegulatorControlScheduleProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorControlScheduleProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorControlScheduleProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorControlScheduleProfile.create());
