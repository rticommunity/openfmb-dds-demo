...
/**
* A sequence of InterconnectionPoint instances.
*/
public final class InterconnectionPointSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InterconnectionPointSeq() {
        super(InterconnectionPoint.class);
    }

    public InterconnectionPointSeq (int initialMaximum) {
        super(InterconnectionPoint.class, initialMaximum);
    }

    public InterconnectionPointSeq (Collection elements) {
        super(InterconnectionPoint.class, elements);
    }

    public  InterconnectionPoint get(int index) { 
        return (InterconnectionPoint) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InterconnectionPoint.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InterconnectionPoint objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InterconnectionPoint.create());
