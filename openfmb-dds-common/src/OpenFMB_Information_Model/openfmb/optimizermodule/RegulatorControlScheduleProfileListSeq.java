...
/**
* A sequence of RegulatorControlScheduleProfileList instances.
*/
public final class RegulatorControlScheduleProfileListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorControlScheduleProfileListSeq() {
        super(RegulatorControlScheduleProfileList.class);
    }

    public RegulatorControlScheduleProfileListSeq (int initialMaximum) {
        super(RegulatorControlScheduleProfileList.class, initialMaximum);
    }

    public RegulatorControlScheduleProfileListSeq (Collection elements) {
        super(RegulatorControlScheduleProfileList.class, elements);
    }

    public  RegulatorControlScheduleProfileList get(int index) { 
        return (RegulatorControlScheduleProfileList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorControlScheduleProfileList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorControlScheduleProfileList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorControlScheduleProfileList.create());
