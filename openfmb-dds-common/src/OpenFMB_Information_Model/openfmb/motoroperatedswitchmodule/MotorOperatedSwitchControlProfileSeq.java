...
/**
* A sequence of MotorOperatedSwitchControlProfile instances.
*/
public final class MotorOperatedSwitchControlProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchControlProfileSeq() {
        super(MotorOperatedSwitchControlProfile.class);
    }

    public MotorOperatedSwitchControlProfileSeq (int initialMaximum) {
        super(MotorOperatedSwitchControlProfile.class, initialMaximum);
    }

    public MotorOperatedSwitchControlProfileSeq (Collection elements) {
        super(MotorOperatedSwitchControlProfile.class, elements);
    }

    public  MotorOperatedSwitchControlProfile get(int index) { 
        return (MotorOperatedSwitchControlProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchControlProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchControlProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchControlProfile.create());
