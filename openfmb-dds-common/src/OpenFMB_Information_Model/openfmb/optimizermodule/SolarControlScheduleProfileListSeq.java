...
/**
* A sequence of SolarControlScheduleProfileList instances.
*/
public final class SolarControlScheduleProfileListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarControlScheduleProfileListSeq() {
        super(SolarControlScheduleProfileList.class);
    }

    public SolarControlScheduleProfileListSeq (int initialMaximum) {
        super(SolarControlScheduleProfileList.class, initialMaximum);
    }

    public SolarControlScheduleProfileListSeq (Collection elements) {
        super(SolarControlScheduleProfileList.class, elements);
    }

    public  SolarControlScheduleProfileList get(int index) { 
        return (SolarControlScheduleProfileList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarControlScheduleProfileList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarControlScheduleProfileList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarControlScheduleProfileList.create());
