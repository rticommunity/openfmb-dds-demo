...
/**
* A sequence of RegulatorReadingTypeKind instances.
*/
public final class RegulatorReadingTypeKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RegulatorReadingTypeKindSeq() {
        super(RegulatorReadingTypeKind.class);
    }

    public RegulatorReadingTypeKindSeq (int initialMaximum) {
        super(RegulatorReadingTypeKind.class, initialMaximum);
    }

    public RegulatorReadingTypeKindSeq (Collection elements) {
        super(RegulatorReadingTypeKind.class, elements);
    }

    public  RegulatorReadingTypeKind get(int index) { 
        return (RegulatorReadingTypeKind) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RegulatorReadingTypeKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RegulatorReadingTypeKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RegulatorReadingTypeKind.create());
