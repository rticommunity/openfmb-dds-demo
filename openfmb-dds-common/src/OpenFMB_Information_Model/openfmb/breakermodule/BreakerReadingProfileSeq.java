...
/**
* A sequence of BreakerReadingProfile instances.
*/
public final class BreakerReadingProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BreakerReadingProfileSeq() {
        super(BreakerReadingProfile.class);
    }

    public BreakerReadingProfileSeq (int initialMaximum) {
        super(BreakerReadingProfile.class, initialMaximum);
    }

    public BreakerReadingProfileSeq (Collection elements) {
        super(BreakerReadingProfile.class, elements);
    }

    public  BreakerReadingProfile get(int index) { 
        return (BreakerReadingProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BreakerReadingProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BreakerReadingProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BreakerReadingProfile.create());
