...
/**
* A sequence of ShuntControlSchedule instances.
*/
public final class ShuntControlScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShuntControlScheduleSeq() {
        super(ShuntControlSchedule.class);
    }

    public ShuntControlScheduleSeq (int initialMaximum) {
        super(ShuntControlSchedule.class, initialMaximum);
    }

    public ShuntControlScheduleSeq (Collection elements) {
        super(ShuntControlSchedule.class, elements);
    }

    public  ShuntControlSchedule get(int index) { 
        return (ShuntControlSchedule) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShuntControlSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShuntControlSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShuntControlSchedule.create());
