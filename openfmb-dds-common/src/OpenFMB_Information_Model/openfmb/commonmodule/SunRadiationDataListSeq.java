...
/**
* A sequence of SunRadiationDataList instances.
*/
public final class SunRadiationDataListSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SunRadiationDataListSeq() {
        super(SunRadiationDataList.class);
    }

    public SunRadiationDataListSeq (int initialMaximum) {
        super(SunRadiationDataList.class, initialMaximum);
    }

    public SunRadiationDataListSeq (Collection elements) {
        super(SunRadiationDataList.class, elements);
    }

    public  SunRadiationDataList get(int index) { 
        return (SunRadiationDataList) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SunRadiationDataList.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SunRadiationDataList objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SunRadiationDataList.create());
