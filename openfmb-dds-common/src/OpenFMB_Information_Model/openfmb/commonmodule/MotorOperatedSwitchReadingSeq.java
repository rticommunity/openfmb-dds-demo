...
/**
* A sequence of MotorOperatedSwitchReading instances.
*/
public final class MotorOperatedSwitchReadingSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchReadingSeq() {
        super(MotorOperatedSwitchReading.class);
    }

    public MotorOperatedSwitchReadingSeq (int initialMaximum) {
        super(MotorOperatedSwitchReading.class, initialMaximum);
    }

    public MotorOperatedSwitchReadingSeq (Collection elements) {
        super(MotorOperatedSwitchReading.class, elements);
    }

    public  MotorOperatedSwitchReading get(int index) { 
        return (MotorOperatedSwitchReading) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchReading.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchReading objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchReading.create());
