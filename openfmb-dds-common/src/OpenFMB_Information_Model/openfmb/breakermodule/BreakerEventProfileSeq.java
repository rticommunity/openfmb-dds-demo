...
/**
* A sequence of BreakerEventProfile instances.
*/
public final class BreakerEventProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BreakerEventProfileSeq() {
        super(BreakerEventProfile.class);
    }

    public BreakerEventProfileSeq (int initialMaximum) {
        super(BreakerEventProfile.class, initialMaximum);
    }

    public BreakerEventProfileSeq (Collection elements) {
        super(BreakerEventProfile.class, elements);
    }

    public  BreakerEventProfile get(int index) { 
        return (BreakerEventProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BreakerEventProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BreakerEventProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BreakerEventProfile.create());
