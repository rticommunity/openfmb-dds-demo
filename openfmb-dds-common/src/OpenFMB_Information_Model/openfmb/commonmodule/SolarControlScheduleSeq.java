...
/**
* A sequence of SolarControlSchedule instances.
*/
public final class SolarControlScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SolarControlScheduleSeq() {
        super(SolarControlSchedule.class);
    }

    public SolarControlScheduleSeq (int initialMaximum) {
        super(SolarControlSchedule.class, initialMaximum);
    }

    public SolarControlScheduleSeq (Collection elements) {
        super(SolarControlSchedule.class, elements);
    }

    public  SolarControlSchedule get(int index) { 
        return (SolarControlSchedule) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SolarControlSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SolarControlSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SolarControlSchedule.create());
