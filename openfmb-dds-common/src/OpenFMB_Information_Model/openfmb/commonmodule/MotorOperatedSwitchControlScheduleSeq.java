...
/**
* A sequence of MotorOperatedSwitchControlSchedule instances.
*/
public final class MotorOperatedSwitchControlScheduleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorOperatedSwitchControlScheduleSeq() {
        super(MotorOperatedSwitchControlSchedule.class);
    }

    public MotorOperatedSwitchControlScheduleSeq (int initialMaximum) {
        super(MotorOperatedSwitchControlSchedule.class, initialMaximum);
    }

    public MotorOperatedSwitchControlScheduleSeq (Collection elements) {
        super(MotorOperatedSwitchControlSchedule.class, elements);
    }

    public  MotorOperatedSwitchControlSchedule get(int index) { 
        return (MotorOperatedSwitchControlSchedule) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorOperatedSwitchControlSchedule.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorOperatedSwitchControlSchedule objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorOperatedSwitchControlSchedule.create());
