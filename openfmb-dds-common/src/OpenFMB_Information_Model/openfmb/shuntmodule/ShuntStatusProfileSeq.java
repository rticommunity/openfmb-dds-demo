...
/**
* A sequence of ShuntStatusProfile instances.
*/
public final class ShuntStatusProfileSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShuntStatusProfileSeq() {
        super(ShuntStatusProfile.class);
    }

    public ShuntStatusProfileSeq (int initialMaximum) {
        super(ShuntStatusProfile.class, initialMaximum);
    }

    public ShuntStatusProfileSeq (Collection elements) {
        super(ShuntStatusProfile.class, elements);
    }

    public  ShuntStatusProfile get(int index) { 
        return (ShuntStatusProfile) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShuntStatusProfile.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShuntStatusProfile objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShuntStatusProfile.create());
